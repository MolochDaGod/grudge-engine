...
bm.diffuseColor = new Color3(0.2, 0.55, 0.2)
bm.specularColor = new Color3(0.4, 0.4, 0.4)
base.material = bm; base.receiveShadows = true

// Load Ninja (ranger class) from R2 RTS character set
SceneLoader.ImportMeshAsync('', `${R2_RTS}/`, 'Ninja_Male.glb', scene)
  .then(r => {
  ...
  .catch(() => {
    ...
    cap.position.y = 0.95
    const m = new StandardMaterial('cm', scene); m.diffuseColor = new Color3(0.2, 0.55, 0.2)
...
document.getElementById('hud')!.textContent = 'Ranger — Ninja class  |  Grudge Warlords  |  Drag to orbit'
